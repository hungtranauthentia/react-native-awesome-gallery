...
import { I18nManager, Image, StyleSheet, View, useWindowDimensions } from 'react-native';
import { Gesture, GestureDetector, GestureHandlerRootView } from 'react-native-gesture-handler';
import Animated, { cancelAnimation, runOnJS, useAnimatedReaction, useAnimatedStyle, useDerivedValue, useSharedValue, withDecay, withSpring, withTiming } from 'react-native-reanimated';
import { useVector } from 'react-native-redash';
import { clamp, resizeImage, withDecaySpring, withRubberBandClamp } from './utils';
...
const defaultRenderImage = ({
  ...
  if (item.type !== GalleryItemType.VIDEO) return /*#__PURE__*/React.createElement(Image, {
    onLoad: e => {
      const {
        height: h,
        width: w
...
const ResizableImage = /*#__PURE__*/React.memo(({
  ...
  const pinchGesture = Gesture.Pinch().enabled(pinchEnabled).onStart(({
    focalX,
    focalY
  }) => {
    'worklet';
  ...
  const panGesture = Gesture.Pan().minDistance(10).maxPointers(1).onBegin(() => {
    'worklet';
    ...
    if (!isVertical.value || scale.value > 1) {
      ...
      if (hideAdjacentImagesOnScaledImage && disableTransitionOnScaledImage) {
        ...
        const moveX = withRubberBandClamp(transX, 0.55, width, disabledTransition ? [getPosition(index), getPosition(index + 1)] : [getPosition(length - 1), 0]);
        ...
        if (loop) {
          ...
          translateX.value = withRubberBandClamp(transX, 0.55, width, disableTransitionOnScaledImage && scale.value > 1 ? [getPosition(index), getPosition(index + 1)] : [getPosition(length - 1), 0]);
        }
  ...
  const tapGesture = Gesture.Tap().enabled(!!onTap).numberOfTaps(1).maxDistance(10).onBegin(() => {
    'worklet';
  ...
  const doubleTapGesture = Gesture.Tap().enabled(doubleTapEnabled).numberOfTaps(2).maxDelay(doubleTapInterval).onEnd(({
    x,
    y,
    numberOfPointers
  }) => {
  ...
  const longPressGesture = Gesture.LongPress().enabled(!!onLongPress).maxDistance(10).onStart(() => {
    'worklet';
  ...
  return /*#__PURE__*/React.createElement(GestureDetector, {
    gesture: (item === null || item === void 0 ? void 0 : item.type) === 'video' ? Gesture.Race(panGesture, tapGesture) : Gesture.Race(Gesture.Simultaneous(longPressGesture, Gesture.Race(panGesture, pinchGesture)), Gesture.Exclusive(doubleTapGesture, tapGesture))
...
const GalleryComponent = ({
  ...
  useAnimatedReaction(() => currentIndex.value, newIndex => runOnJS(changeIndex)(newIndex), [currentIndex, changeIndex]);
  useEffect(() => {
  ...
  useImperativeHandle(ref, () => ({
    setIndex(newIndex, animated) {
      ...
      if (animated) {
        translateX.value = withSpring(newIndex * -(dimensions.width + emptySpaceWidth), springConfig);
      } else {
  ...
  }, [data === null || data === void 0 ? void 0 : data.length, dimensions.width]);
  return /*#__PURE__*/React.createElement(GestureHandlerRootView, {
    ...
    const isFirst = i === 0;
    const renderCount = item.type === GalleryItemType.VIDEO ? 1 : numToRender;
    const outOfLoopRenderRange = !isLoop || Math.abs(i - index) < data.length - (renderCount - 1) / 2 && Math.abs(i - index) > (renderCount - 1) / 2;
    const hidden = Math.abs(i - index) > (renderCount - 1) / 2 && outOfLoopRenderRange;
    console.log(hidden);
